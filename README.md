...
| 文件 | 说明 |
|---|---|
| `index.html` | 站点外壳：顶部标题栏 + 左侧目录 + 右侧正文（iframe 双栏，模仿 CHM 查看器）；**没有独立首页**，打开即在右栏显示目录第一篇正文（`page_241.html`） |
| `toc.html` | 由 CHM 的 `.hhc` 生成的目录树（2406 节点，可折叠、可过滤，点击在右栏打开） |
| `404.html` | 自定义 404 页：说明 404 的常见来源并给出回程入口 |
| `spellindex.html` + `class-*.html` | **万法大全速查**：1919 条法术 + 27 个职业速查页（从左侧目录「常用速查」进入） |
| `featindex.html` | **专长大全速查**：1226 条专长 |
| `creatureindex.html` | **生物大全速查**：379 条生物 |
| `*.lookup.html` | 21 个原文伴生页（原文逐字节副本 + 条目锚点） |
| 其余 `.html` / `.htm` | 原书全部页面（2148 个），内容一字未改，仅转码为 UTF-8 |
